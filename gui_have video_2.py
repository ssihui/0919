...
def show_chart():
    ...
    try:
        ...
        messagebox.showerror("錯誤", f"未找到文件: {json_file}")
        return

    chart_window = Toplevel(root)
    chart_window.title("歷史資料")
    ...
    user_dropdown = tk.OptionMenu(chart_window, textvariable=selected_user_var, values=users, state='readonly')
    ...
    date_dropdown =tk.OptionMenu(chart_window, textvariable=selected_date_var, state='readonly')
    ...
    user_dropdown.bind("<<ComboboxSelected>>", lambda event: update_dates(data, user_dropdown.get(), date_dropdown, ai_response_label, canvas))

    selected_user = users[0]
    user_dates = get_user_dates(data, selected_user)

    date_dropdown['values'] = user_dates
    date_dropdown.set(user_dates[0]) 

    update_chart_and_ai(data, selected_user, user_dates[0], canvas, ai_response_label)

    date_dropdown.bind("<<ComboboxSelected>>", lambda event: update_chart_and_ai(data, user_dropdown.get(), date_dropdown.get(), canvas, ai_response_label))
...
# 更新图表和显示AI建议
def update_chart_and_ai(data, selected_user, selected_date):
    # 查找与所选日期和用户匹配的记录
    selected_entry = None
    for entry in data:
...
# 用户选择后更新日期选项
def update_dates(data, selected_user):
    # 获取该用户的所有日期
    user_dates = get_user_dates(data, selected_user)
    ...
    if user_dates:
        # 更新日期下拉菜单的选项
        date_dropdown['values'] = user_dates
        date_dropdown.set(user_dates[0])  # 默认选择最新日期

        # 显示最新日期的数据和图表
        update_chart_and_ai(data, selected_user, user_dates[0])
    else:
        date_dropdown['values'] = []
        date_dropdown.set("无可用日期")

# 显示图表的主功能
def show_chart():
    json_file = 'user_data.json'  # 请确保该文件存在
    ...
    # 创建用户选择下拉菜单
    user_label = Label(root, text="选择用户:")
    user_label.pack(pady=5)
    
    selected_user_var = StringVar()
    user_dropdown = ttk.Combobox(root, textvariable=selected_user_var, values=users, state='readonly')
    user_dropdown.pack(pady=5)
    user_dropdown.set(users[0])  # 默认选择第一个用户

    # 当选择用户时，更新日期下拉菜单
    user_dropdown.bind("<<ComboboxSelected>>", lambda event: update_dates(data, user_dropdown.get()))
    ...
    # 创建日期选择下拉菜单
    date_label = Label(root, text="选择日期:")
    date_label.pack(pady=5)
    
    selected_date_var = StringVar()
    global date_dropdown
    date_dropdown = ttk.Combobox(root, textvariable=selected_date_var, values=user_dates, state='readonly')
    date_dropdown.pack(pady=5)
    date_dropdown.set(user_dates[0])  # 默认选择最新日期
    ...
    # 当选择日期时，更新图表和AI建议
    date_dropdown.bind("<<ComboboxSelected>>", lambda event: update_chart_and_ai(data, user_dropdown.get(), date_dropdown.get()))
